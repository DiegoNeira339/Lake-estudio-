...
export default function AdminDashboard() {
  ...
  const [vista, setVista] = useState('reservas'); // 'reservas', 'galeria' o 'servicios'
  ...
  const [formServicio, setFormServicio] = useState({ id: null, titulo: '', descripcion: '', foto_url: '' });
  ...
  const reservasMes = reservas.filter(r => r.fecha_hora && new Date(r.fecha_hora).getMonth() === mesActual && new Date(r.fecha_hora).getFullYear() === anioActual);
  const totalCitasMes = reservasMes.length;
  ...
  const login = async (e) => { 
    ...
    try {
      ...
      if (res.ok) {
        ...
        Swal.fire('Alto ahí', 'Clave incorrecta o intruso detectado 🛑', 'error');
      }
    } catch (error) {
  ...
  const subirFotoGaleria = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        setFotos([{ id: Date.now(), url: data.url }, ...fotos]);
        Swal.fire('¡Éxito!', 'La foto se subió a la galería', 'success');
      } else throw new Error();
    } catch (error) {
      Swal.fire('Error', 'Hubo un problema subiendo la foto', 'error');
    } finally {
  ...
  const eliminarFotoGaleria = async (id, url) => {
    ...
    if (!isConfirmed) return;
    if ((await fetch('/api/galeria', { method: 'DELETE', body: JSON.stringify({ id, url }) })).ok) {
      setFotos(fotos.filter(f => f.id !== id));
      Swal.fire('Eliminada', 'La foto se borró', 'success');
    }
  };

  // ================= FUNCIONES SERVICIOS =================
  const resetFormServicio = () => {
    setFormServicio({ id: null, titulo: '', descripcion: '', foto_url: '' });
  ...
  const guardarServicio = async (e) => {
    ...
    try {
      ...
      Swal.fire('Error', 'Hubo un problema guardando el servicio', 'error');
    } finally {
  ...
  const eliminarServicio = async (id) => {
    ...
    if (!isConfirmed) return;
    if ((await fetch('/api/servicios', { method: 'DELETE', body: JSON.stringify({ id }) })).ok) {
      setServicios(servicios.filter(s => s.id !== id));
      Swal.fire('Eliminado', 'El servicio se borró', 'success');
    }
  };
    ...
    <div className="min-h-screen bg-lake-white p-6 md:p-12 relative text-lake-dark font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="flex gap-4 mb-8 border-b-2 border-gray-200 pb-4 overflow-x-auto scrollbar-hide">
          <button onClick={() => setVista('reservas')} className={`font-bold px-6 py-2 rounded-full transition-colors whitespace-nowrap ${vista === 'reservas' ? 'bg-lake-dark text-white shadow-md' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
            📅 Citas
          </button>
          <button onClick={() => setVista('galeria')} className={`font-bold px-6 py-2 rounded-full transition-colors whitespace-nowrap ${vista === 'galeria' ? 'bg-lake-dark text-white shadow-md' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
            📸 Galería
          </button>
          <button onClick={() => setVista('servicios')} className={`font-bold px-6 py-2 rounded-full transition-colors whitespace-nowrap ${vista === 'servicios' ? 'bg-lake-dark text-white shadow-md' : 'bg-gray-200 text-gray-600 hover:bg-gray-300'}`}>
            ✨ Servicios
          </button>
        </div>
            ...
            {vista === 'reservas' && (
              <div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
                  <div className="bg-lake-pink/20 border-2 border-lake-pink rounded-3xl p-6 flex items-center gap-4 shadow-sm">
                    <div className="text-4xl">📅</div><div><p className="text-sm text-gray-600 font-bold uppercase">Citas este mes</p><p className="text-3xl font-extrabold text-lake-dark">{totalCitasMes}</p></div>
                  </div>
                  <div className="bg-lake-matcha/20 border-2 border-lake-matcha rounded-3xl p-6 flex items-center gap-4 shadow-sm">
                    <div className="text-4xl">✨</div><div><p className="text-sm text-gray-600 font-bold uppercase">Servicio Estrella</p><p className="text-2xl font-extrabold text-lake-dark capitalize truncate">{servicioPopular}</p></div>
                  </div>
                  <div className="bg-blue-100 border-2 border-blue-200 rounded-3xl p-6 flex items-center gap-4 shadow-sm">
                    <div className="text-4xl">💸</div><div><p className="text-sm text-gray-600 font-bold uppercase">Proyección Mes</p><p className="text-2xl font-extrabold text-lake-dark">{gananciasFormateadas}</p></div>
                  </div>
                </div>
              ...
              <div className="bg-white rounded-3xl p-8 shadow-sm border-2 border-gray-100">
                <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                  <div>
                    ...
                    <p className="text-gray-500">Sube fotos para mostrar en la web (ideal cuadradas).</p>
                  </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {fotos.length === 0 && !subiendoGaleria ? <p className="col-span-full text-center text-gray-500 py-10">Aún no hay fotos en la galería.</p> : null}
                  {fotos.map(foto => (
                    <div key={foto.id} className="relative group rounded-xl overflow-hidden aspect-square border-2 border-gray-100 shadow-sm">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button onClick={() => eliminarFotoGaleria(foto.id, foto.url)} className="bg-red-500 text-white font-bold py-2 px-4 rounded-lg hover:bg-red-600 transition-colors shadow-lg">
                          🗑️ Borrar
                        </button>
                      </div>
                    </div>
            ...
            {vista === 'servicios' && (
              <div className="bg-white rounded-3xl p-8 shadow-sm border-2 border-gray-100">
                ...
                <form onSubmit={guardarServicio} className="mb-12 bg-lake-pink/10 p-6 rounded-3xl border-2 border-lake-pink/30">
                  ...
                  <div className="grid md:grid-cols-2 gap-4">
                    ...
                    <div className="relative">
                      ...
                      {editandoServicio && !fotoServicio && <span className="absolute right-4 top-3.5 text-xs text-gray-400">Deja vacío para mantener la foto actual</span>}
                    </div>

                    <textarea placeholder="Descripción del servicio (Mencionando detalles, pasos, etc.)" value={formServicio.descripcion} onChange={e => setFormServicio({...formServicio, descripcion: e.target.value})} className="border-2 p-3 rounded-xl w-full md:col-span-2 outline-none focus:border-lake-pink bg-white resize-none shadow-sm" required rows="3"></textarea>
                  </div>

                  <div className="mt-6 flex gap-3">
                    {editandoServicio && (
                      <button type="button" onClick={resetFormServicio} className="w-1/3 bg-gray-200 text-gray-700 py-3 rounded-xl font-bold hover:bg-gray-300 transition-colors shadow-sm">
                        Cancelar
                      </button>
                    )}
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {servicios.length === 0 ? <p className="col-span-full text-center text-gray-500 font-medium">No has creado ningún servicio todavía.</p> : (
                    servicios.map(s => (
                      <div key={s.id} className="border-2 border-gray-100 rounded-3xl overflow-hidden shadow-sm hover:border-lake-matcha transition-colors flex flex-col h-full bg-white group">
                        <div className="h-48 relative bg-gray-100 overflow-hidden">
                          {s.foto_url ? (
                            <img src={s.foto_url} alt={s.titulo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                          ) : (
                            <div className="flex items-center justify-center h-full text-gray-400 font-medium">Sin foto 📷</div>
                          )}
                        </div>
                        <div className="p-6 flex flex-col flex-grow">
                          <h4 className="font-bold text-xl mb-2 capitalize text-lake-dark">{s.titulo}</h4>
                          <p className="text-sm text-gray-600 mb-6 flex-grow">{s.descripcion}</p>
                          <div className="flex gap-2 mt-auto">
                            <button onClick={() => { setFormServicio(s); setEditandoServicio(true); setFotoServicio(null); window.scrollTo({ top: 0, behavior: 'smooth' }); }} className="flex-1 bg-blue-100 text-blue-700 py-2.5 rounded-xl font-bold text-sm hover:bg-blue-200 transition-colors shadow-sm">Editar</button>
          ...
          <div className="bg-white rounded-3xl p-8 max-w-md w-full relative max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            {editando ? (
              ...
              <>
                ...
                {sel.whatsapp && (
                  <a href={linkWA(sel.whatsapp)} target="_blank" rel="noopener noreferrer" className="block text-center bg-green-500 text-white font-bold py-3 rounded-xl hover:bg-green-600 transition-colors shadow-md">
